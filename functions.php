...
function ebook_wp_custom_attachment() {
	...
	$query = new WP_Query('post_type=ebook');
	$ebook_bonus_options = '';
	$ebook_bonus_options = "<option>Please select</option>";
	$ebook_bonus_meta = get_post_meta($ebook_id, 'ebook_bonus',true);
	//wp_die($ebook_bonus_meta);
		while ( $query->have_posts() ) {
			$query->the_post();
			if (get_the_ID() != $ebook_id) {
				$selected = '';
				if (get_the_ID() == $ebook_bonus_meta) { 
...
<p>'.__('Price', 'ebook-store').' <span style="color:red; font-size:15px;"> * </span><br /><input name="ebook[ebook_price]" placeholder="0.00" min="0" type="number" step="any" value="' . @$ebook['ebook_price'] . '"></p>
...
$html .= '<h3>'.__('Bonus ebook', 'ebook-store').'</h3>';
$html .= '<select name="ebook_bonus">';
...
function ebook_download_link($ebook_order, $free = false, $bonus = false) {
	...
	if ($bonus > 0) {
		...
		$link = add_query_arg(array('ebook_id' => $ebook_order['ebook'][0]),$link);
	}
	//die('Bonus ' . $bonus);
	return $link;
}
...
function ebook_store( $atts, $buyNowOnly = false ){
	...
	$buyNowLinkText = $locale['buy'] . ' (' . $c->getSymbol($paypal_currency) . @number_format($ebook['ebook_price'],2) . ')';
...
function ebook_process_download($data = false) {
    ...
    setInterval(\'window.location.reload()\', 15000);
		...
		$strtotime = strtotime("+" . get_option('link_expiration'),$gmt_timestamp);
		...
		while ( $loop->have_posts() ) : $loop->the_post();
			...
			if (get_option('encrypt_pdf') == 1) {
				if (!ebook_store_encryptable($attachment[0]['file'])) {
					echo('The uploaded file is not in the proper format, please re-upload it in Acrobat 5.0 PDF 1.5 Compatibility mode.');
				}
			}
			//die();
		...
		header("Location: " . ebook_download_link($ebook_order, 0, 1)); //add proper ebook id here and allow it.
	}
}
...
function ebook_get_order($key = 'ebook_key',$val) {
	...
	@$meta['total'][0] = round($meta['mc_gross'][0] + $meta['tax'][0],2);
	return $meta;
	endwhile;
}
...
function ebook_download_links_bonus($ebook_order, $free = 0) {
	$ebook_id = $ebook_order['ebook'][0];
	$ebook_bonus = get_post_meta($ebook_id, 'ebook_bonus', true);
	$links = array();
	if ($ebook_bonus > 0) {
		$post = get_post($ebook_bonus);
		$ebook_key = (strlen($ebook_order['ebook_key'][0]) == 32 ? $ebook_order['ebook_key'][0] : $ebook_order['ebook_key_array'][0]);
		$link = add_query_arg(array('ebook_key' => $ebook_key,'ebook_id' => $ebook_id,'action' => 'ebook_bonus_download', 'md5_nonce' => @$ebook_order['md5_nonce'][0]),get_permalink($ebook_bonus));
...
function ebook_store_add_order($data, $silent = false) {
...
function ebook_store_woocommerce_email_delivery( $order_status, $order_id ) {
	...
	$ebook_email_delivery = array(
		...
		$ebook_email_delivery['order'] = array(
			...
			'mc_currency' => $order->get_order_currency(),
...
function ebook_store_woocommerce_order_details($order){
	$order_items = $order->get_items();
			...
			@$out .= '
				<tr class="order_item">
					...
					<td class="product-total">
						<span class="amount">' . $ebook->format_links() . '</span><br /><small>' . $ebook->password_desc() . '</small></td>
				</tr>';
	...
	if (ebook_store_endsWith($_SERVER['REQUEST_URI'], 'edit.php?post_type=ebook')) {
	   ...
	   'post_type' => 'ebook',
        ...
        <p><?php _e( 'You have not created any ebooks yet, do you want to see the sample videos how to help you get started with it?<br />
        	<a href="https://www.youtube.com/watch?v=T0mrdvrbmbc&list=PL9hp_4MJ0h1sDTf05H6aguzlYqcg47QJ-" target="_blank">Watch Tutorials on YouTube</a>', 'ebooks-store' ); ?></p>
    </div>
    		<?php
		}
	}	
...
function ebook_store_not_encryptable() {
	...
	$message = __( 'This file is can not be encrypted. Please save it in Adobe Acrobat 5.0 PDF 1.5 compatibility format.', 'ebook-store' );
		...
		foreach ($formats as $format) {
			...
			if ($format == 'pdf') {
				if (get_option('encrypt_pdf')) {
					if (!ebook_store_encryptable($ebook->files[$format])) {
			...
			<div class="ebook_store_file_format_item <?php echo ($i > 1 ? 'goPro2' : ''); ?>">
				...
				<div class="ebook_store_file_format_item_right">
					...
					<span class="ebook_store_upload"><input type="file" name="ebook_wp_custom_attachment_<?php echo $format; ?>"></span>
